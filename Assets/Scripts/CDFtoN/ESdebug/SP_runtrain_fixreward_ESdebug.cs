...
public class SP_runtrain_fixreward_ESdebug : MonoBehaviour
{
    ...
    private RR_runtrain_fixreward_ESdebug rr;
    private DL_runtrain_fixreward_ESdebug dl;
    private PC_runtrain_fixreward_ESdebug pc;
    private SbxTTLs_runtrain_fixreward_ESdebug ttls;
    private Notes notes;
    ...
    public void Awake()
    {
        ...
        rr = player.GetComponent<RR_runtrain_fixreward_ESdebug>();
        dl = player.GetComponent<DL_runtrain_fixreward_ESdebug>();
        pc = player.GetComponent<PC_runtrain_fixreward_ESdebug>();
        ttls = player.GetComponent<SbxTTLs_runtrain_fixreward_ESdebug>();
        ...
        "tstart INT, teleport INT, rzone INT, scanning NUMERIC, manrewards INT, autoreward INT, cmd INT, trainingtrack INT, blackout INT)";
    ...
    void LateUpdate()
    {
        ...
        "tstart, teleport, rzone , scanning, manrewards, autoreward, cmd, blackout) values (" + Time.realtimeSinceStartup + "," + morph + "," + TrainingTrack + "," + numTraversals +
        "," + transform.position.z + "," + rr.true_delta_z + "," + transform.position.x + "," + dl.c_1 + "," + dl.r + "," + pc.tstartFlag + "," + pc.tendFlag + "," +
        pc.rzoneFlag + "," + ttls.scanning + "," + pc.mRewardFlag + "," + _autoReward + "," + pc.cmd + "," + Convert.ToByte(pc.blackoutActive) +")";
